...
public class UserService {
    ...
    public User register(RegisterRequest request) {
        ...
        PlaceOfResidence residence =
            repo.findAll().stream()
                ...
                .findFirst()
                .orElseGet(() -> PlaceOfResidence.builder()
                    .city(request.city())
    ...
    public UserProfileResponse getUserProfile(Integer id) {
        ...
        return new UserProfileResponse(
            ...
            user.getPosition(),
            user.getStartOfEmployment() != null
                ? user.getStartOfEmployment().toString()
                : null,
    ...
    public User updateUser(Integer id, UserUpdateRequest request) {
        ...
        if (request.dateOfBirth() != null && !request.dateOfBirth().isBlank()) {
            LocalDate dob = LocalDate.parse(request.dateOfBirth(), FORMATTER);
    ...
    public List<User> getUsers(UserSearchRequest request) {
        ...
        if (request.firstName() != null && !request.firstName().isBlank()) {
            spec = spec.and((root, query, cb) ->
                    cb.equal(root.get("firstName"), request.firstName()));
        }

        if (request.lastName() != null && !request.lastName().isBlank()) {
            spec = spec.and((root, query, cb) ->
                    cb.equal(root.get("lastName"), request.lastName()));
        }
